...
| 締め日 | 月末 |
| 支払日 | 翌月10日（手渡し） |
| 深夜・休日手当 | 深夜手当あり（22:00〜翌5:00は時給1.25倍）、休日手当なし |
| 税金・社会保険控除 | なし（対象外） |
...
- 2店舗の勤務時間を合算
- 時給は**スタッフ×店舗ごとに設定**
- 計算式: `店舗ごとの通常勤務時間 × 店舗別時給 + 店舗ごとの深夜勤務時間 × 店舗別時給 × 1.25` の合計 + 交通費
- 給与管理画面を開いた時・対象月を切り替えた時に、未確定の給与を自動で再計算する
- 確定済み給与は自動再計算では更新せず、管理者が明示的に再計算した場合のみ更新する
- 年末調整用に、スタッフ本人と管理者が年単位の給与合計・月別内訳を確認できる
